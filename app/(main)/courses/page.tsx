...
async function getCourses(level?: string, search?: string): Promise<CourseListing[]> {
  try {
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {foundationCourses.map((course) => {
                    const lessonCount = (course.modules as { lessons: unknown[] }[]).reduce((sum, m) => sum + m.lessons.length, 0)
                    return (
                       <CourseCard
                         key={course.id}
                         id={course.id}
